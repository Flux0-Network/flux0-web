...
class CodeModal(discord.ui.Modal):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        cursor.execute(
            "SELECT code, verified FROM users WHERE discord_id = ?",
            (user_id,)
        )
...
class VerifyView(discord.ui.View):
    ...
    @discord.ui.button(
        ...
        custom_id="VerifyView_enter_code"
    )
...
class AccountView(discord.ui.View):
    ...
    async def create_account(self, button, interaction: discord.Interaction):
        ...
        cursor.execute(
            ...
            (user_id, account_id, code, str(datetime.utcnow()))
        )
        conn.commit()

        try:
            await interaction.user.send(
                f"🔐 Dein Verifizierungs-Code:\n`{code}`"
            )
    ...
    async def resend_code(self, button, interaction: discord.Interaction):
        ...
        try:
            ...
            await interaction.response.send_message(
                "📩 Code wurde dir erneut geschickt!",
                ephemeral=True
            )
        except Exception:
            await interaction.response.send_message(
...
async def setup(ctx: discord.ApplicationContext):
    embed = discord.Embed(
        description=(
            "# <:account:1474536531061379113> Account\n"
            "\n"
...
async def account(ctx: discord.ApplicationContext):
    cursor.execute(
        "SELECT account_id, verified FROM users WHERE discord_id = ?",
        (str(ctx.user.id),)
    )
